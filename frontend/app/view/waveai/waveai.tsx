...
export class WaveAiModel implements ViewModel {
    ...
    constructor(blockId: string, nodeModel: BlockNodeModel, tabModel: TabModel) {
        ...
        this.viewText = atom((get) => {
                    ...
                    if (isCloud) {
                        ...
                        if (baseUrl.startsWith("http://localhost") || baseUrl.startsWith("http://127.0.0.1")) {
                            viewTextChildren.push({
                                ...
                                icon: "location-dot",
                                title: t(
                                    "waveAi.usingLocalModel",
                                    { baseUrl, modelName },
                                    "Using Local Model @ {{baseUrl}} ({{modelName}})"
                                ),
                                noAction: true,
                            });
                        } else {
                            viewTextChildren.push({
                                ...
                                icon: "globe",
                                title: t(
                                    "waveAi.usingRemoteModel",
                                    { baseUrl, modelName },
                                    "Using Remote Model @ {{baseUrl}} ({{modelName}})"
                                ),
                                noAction: true,
                            });
                        }
            ...
            dropdownItems.push({
                label: t("waveAi.addPreset", undefined, "Add AI preset..."),
                onClick: () => {
                    fireAndForget(async () => {
            ...
            viewTextChildren.push({
                elemtype: "menubutton",
                text: presetName,
                title: t("waveAi.selectConfiguration", undefined, "Select AI Configuration"),
                items: dropdownItems,
            });
        ...
        this.endIconButtons = atom((_) => {
            let clearButton: IconButtonDecl = {
                elemtype: "iconbutton",
                icon: "delete-left",
                title: t("waveAi.clearChatHistory", undefined, "Clear Chat History"),
    ...
    ({ value, onChange, onKeyDown, onMouseDown, baseFontSize, model }, ref) => {
        ...
        return (
            <textarea
                ...
                placeholder={t("waveAi.askAnything", undefined, "Ask anything...")}
                value={value}
            ></textarea>
        );
    }
